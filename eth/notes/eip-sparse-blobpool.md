...
**Sampler role.**
If the node is a sampler, it SHOULD await to observe at least 2 distinct provider announcements before acting. It then MUST request cells matching its custody columns from peers that announced overlapping availability. This includes providers. The node MUST only request a maximum of `C_req` columns per request. When fetching from a provider, the node MUST request `C_extra` random columns in addition to its custody set.
